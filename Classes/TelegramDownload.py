...
async def dir_specify(update: Update, context: CallbackContext):
    ...
    if (update.callback_query):
        ...
        await update.callback_query.message.reply_text(f'{await get_path(context)}{message}')
    else:
        await set_path(context, update.message.text)
        await update.message.reply_text(f'{await get_path(context)}{message}')
    await ls_command(update, context)
    return SPECIFY
...
async def ls_command(update: Update, context: CallbackContext):
    ...
    message = 'select or send the next directory name (for create or move)'
    directories = PathManage.GetInlineAllDirectories(await get_path(context))
    if (update.callback_query):
        await update.callback_query.message.reply_text(message, reply_markup=InlineKeyboardMarkup(
...
async def dw_list(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text("\n".join(await dataManage.get_view_download_list()))
